...
- A GitHub organization (referred to as `myorg` throughout this guide)
...
gh repo create myorg/sso-mocker \
...
git remote add origin https://github.com/myorg/sso-mocker.git
```
...
Each repo that uses `@myorg/sso-mocker` needs a `.npmrc` file:
...
1. Go to `https://github.com/orgs/myorg/packages/npm/sso-mocker/settings`
...
1. Go to `https://github.com/organizations/myorg/settings/packages`
...
1. Go to `https://github.com/orgs/myorg/packages/container/sso-mocker/settings`
...
1. Go to `https://github.com/myorg/sso-mocker/settings/branches`
...
gh api repos/myorg/sso-mocker/branches/main/protection \
  --method PUT \
...
GitHub org-level Pages are served from a repo named `myorg.github.io`:

```bash
# Check if it exists
gh repo view myorg/myorg.github.io 2>/dev/null || \
  gh repo create myorg/myorg.github.io --public --clone
```
...
1. Go to `https://github.com/myorg/myorg.github.io/settings/pages`
...
```bash
curl https://myorg.github.io/.well-known/openid-configuration
```
...
```bash
curl -I https://myorg.github.io/.well-known/openid-configuration 2>/dev/null | grep content-type
```
...
2. Commits it to the `myorg.github.io` repo
3. GitHub Pages auto-deploys

To set this up, the sso-mocker repo needs a PAT or deploy key with write access to `myorg.github.io`:

```bash
# Create a fine-grained PAT with contents:write on myorg.github.io repo
# Then add it as a secret in the sso-mocker repo:
gh secret set PAGES_DEPLOY_TOKEN --body "ghp_your_token_here" --repo myorg/sso-mocker
```
...
1. Go to `https://github.com/organizations/myorg/settings/actions`
...
1. Go to `https://github.com/myorg/sso-mocker/settings/actions`
2. Under "Workflow permissions", select **Read and write permissions**
...
1. Go to `https://github.com/myorg/sso-mocker/settings/environments`
...
1. Publishes `@myorg/sso-mocker@0.1.0` to GitHub Packages (npm)
2. Builds and pushes `ghcr.io/myorg/sso-mocker:v0.1.0` and `ghcr.io/myorg/sso-mocker:latest` to GHCR

### 10c. Verify the release

```bash
# Check npm package
npm view @myorg/sso-mocker --registry=https://npm.pkg.github.com

# Check Docker image
docker pull ghcr.io/myorg/sso-mocker:v0.1.0
docker run -p 9090:9090 ghcr.io/myorg/sso-mocker:v0.1.0
curl http://localhost:9090/.well-known/openid-configuration
...
```bash
npm install --save-dev @myorg/sso-mocker
```
...
```bash
gh repo view myorg/.github 2>/dev/null || \
  gh repo create myorg/.github --public
...
jobs:
  sso-mocker:
    ...
    services:
      sso-mocker:
        image: ghcr.io/myorg/sso-mocker:${{ inputs.mocker-version }}
        credentials:
          username: ${{ github.actor }}
          password: ${{ secrets.GITHUB_TOKEN }}
        env:
...
jobs:
  test:
    uses: myorg/.github/.github/workflows/sso-mocker-setup.yml@main
    with:
      login-mode: form
...
- [ ] `gh repo view myorg/sso-mocker` shows the repo
...
- [ ] npm package is published to `https://github.com/orgs/myorg/packages/npm/sso-mocker`
- [ ] Docker image is published to `https://github.com/orgs/myorg/packages/container/sso-mocker`
- [ ] `https://myorg.github.io/.well-known/openid-configuration` returns the discovery JSON
- [ ] A consuming repo can pull the Docker image in GitHub Actions
- [ ] A consuming repo can install the npm package via `@myorg/sso-mocker`
- [ ] Local dev works: `npx @myorg/sso-mocker start` or `docker run ghcr.io/myorg/sso-mocker:latest`
